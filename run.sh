...
build() {
  ...
  mkdir -p "$SIM_DIR"
  "$IVERILOG" $IVERILOG_FLAGS -I"$RTL_DIR" -o "$SIM_OUTPUT" "$TB_FILE" "${RTL_SOURCES[@]}"
...
sim() {
  ...
  build
  local sim_log="$SIM_DIR/sim.log"
  "$VVP" "$SIM_OUTPUT" | tee "$sim_log"
  if grep -q "^FAIL:" "$sim_log"; then
    echo -e "${RED}error:${NC} simulation reported FAIL (see $sim_log)" >&2
    exit 1
  fi
